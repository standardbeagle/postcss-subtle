 ...
 * @type {Plugin<T>}
 */
var subtle = _postcss2.default.plugin('postcss-subtle', function (local_directory, server_directory) {
    ...
    return function (style, result) {
        walkStream(style.walkDecls.bind(style)).filter(decl => longEmTest.test(decl.value)).flatMap(decl => {
            ...
            return walkStream(parsed.walk.bind(parsed)).filter(node => node.type === 'function' && node.value === 'subtle').flatMap(node => {
                ...
                return _kefir2.default.fromNodeCallback(cb => _fs2.default.stat(local_path, cb)).flatMapErrors(() => downloadBuffer(download_url).flatMap(buffer => _kefir2.default.fromPromise(_jszip2.default.loadAsync(buffer))).map(zip => zip.file(/png$/i)).flatten().filter(file => file.name.indexOf('MACOSX') < 0).take(1).flatMap(file => _kefir2.default.fromEvents(file.nodeStream().pipe(_fs2.default.createWriteStream(local_path)), 'finish')));
            });
        }).onAny(x => x);
    };
});
